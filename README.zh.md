...
**先开口，才记账。** 终端程序一启动就会写下会话头，所以一个开了却没说过话的终端，也会在磁盘上留下一份货真价实、只是很小的日志。host 把这样的对话读成 `blank`——而 `blank` 不只是"侧边栏里不显示"这一层意思，它是框架用来说 *这段对话可以被"新建会话"复用* 的那个词：`workspaces.connectWorkspace` 会在工作区的账目里找出这样一段并打开它。于是一段没有轮次的 Code 对话只要留在账目里，用户下次在 Chat 或 Work 里按"新建会话"，拿回来的就是它——会话列变成终端，模式在用户手底下被换掉；框架启动时选默认工作区走的也是同一条复用，所以整个页面都可能一打开就落在一个没人要的终端里。而这件事没有任何值得权衡的收益：侧边栏本来就不画 blank 的行。所以账目跟着对话走，且是双向的：里面跑过一轮，就挂上；一轮都没跑过，就摘掉。
...
在第一轮对话落盘之前，这段新对话没有行，所以侧边栏仍然显示你来时的那一行。那一轮落盘之后，行会自己出现——而选中状态不会跟着过去，因为一段 Code 对话是**被显示的，而不是被选中的**：把它变成运行时的当前会话，正是让这个 host 去恢复它的那个动作，而它的终端还在往那份日志里追加。代价是侧边栏的高亮：它会留在终端背后那段网页对话上。
